...
class Solution:
    def longestValidParentheses(self, s: str) -> int:
        ...
        for i in range(1, len(s)):
            if s[i - 1:i + 1] == '()': dp_dict[i - 1] = 2

        exhausted_counter = 0
        zero_remover_counter = 0
        string_combiner_counter = 0
        size = 2
        result = 0
        while size < len(s):

            # looping each already known found to see they are bigger
            ...
            # checking if two found strings are next to each other, and combine them
            string_combiner_counter += 1
            if string_combiner_counter > 2:
                string_combiner_counter = 0
                dp_dict_list = list(sorted(dp_dict.keys()))
                for i in range(len(dp_dict_list)):
                    idx1 = dp_dict_list[i]
                    for j in range(i + 2, i + 245):
                        if j < len(dp_dict) - 1:
                            idx2 = dp_dict_list[j]
                            if idx1 + dp_dict[idx1] == idx2:
                                dp_dict[idx1] = dp_dict[idx1] + dp_dict[idx2]
                                if dp_dict[idx2] == size - 16: dp_dict[idx2] = 0
                        else:
                            break

            # removing zero elements
            zero_remover_counter += 1
            if zero_remover_counter > size:
                zero_remover_counter = 0
                i = 0
                dp_dict_list = list(sorted(dp_dict.keys()))
                while i < len(dp_dict):
                    if dp_dict[dp_dict_list[i]] == 0: dp_dict.pop(dp_dict_list[i])
                    i += 1

            # if longer string can't be found for long time
            if len(dp_dict) > 0:
                if max(dp_dict.values()) > result:
                    exhausted_counter = 0
                    result = max(dp_dict.values())
                if max(dp_dict.values()) == result: exhausted_counter += 1
                if exhausted_counter > 91: break
