...
for blockchain0 in blockchain_list:
    ...
    str0 = f"cd {path} && . ./activate && {blockchain0[0]} start wallet-only"
    with Halo(text=f'starting {blockchain0[0]} wallet', color='white'):
        subprocess.getoutput(str0)

    fingerprints = []
    str0 = f"cd {path} && . ./activate && {blockchain0[0]} keys show | grep Fingerprint"
    ...
    for fingerprint in fingerprints:
        ...
        str0 = f"cd {path} && . ./activate && {blockchain0[0]} wallet show -f {fingerprint}"
    ...
    str0 = f"cd {path} && . ./activate && {blockchain0[0]} stop wallet-only"
